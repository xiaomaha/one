...
class DurationStatisticController extends Controller
{
    use ModelForm;
    protected $header = '区域活动统计';
     ...
     * @return array
     */
    public function getDurationDataByClass()
    {
        $my_data = array();
        array_push($my_data, RFIDRead::all()->count());
        array_push($my_data,'3');
        array_push($my_data,'8');
        array_push($my_data,'1');
        ...
        return Admin::content(function (Content $content) {
            ...
            $content->row(function (Row $row) {
                ...
                $row->column(8, function (Column $column) {
                    $column->append(view('admin.regional_activity.statistic_by_class'));
                });
            });
    ...
    protected function grid()
    {
        return Admin::grid(RFIDRead::class, function (Grid $grid) {
            ...
            $grid->column('card_no','学生姓名')->sortable()->display(function ($student_card_no){
                ...
                if(isset($student)){
                    $result= $student->name;
                    $model =SchoolClass::find($student->class_id);
                    if(isset($model)){
                        return $model->full_name.' '.$result;
                    }else{
                        return '';
                    }
                }
